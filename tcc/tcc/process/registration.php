<?php
    ...
    if (isset($_POST["username"]) && isset($_POST["email"]) && isset($_POST["password"]) && isset($_POST["repPassword"]) && isset($_POST["cpf"])) {
        ...
        $stmt = $con->prepare("INSERT INTO User (`Username`, `Email`, `Password`, `CPF`, `Online`, `Token`, `Secure`, `Creation`) 
        VALUES (?, ?, ?, ?, now(), ?, ?, now())");
$stmt->bind_param("sssssi", $username, $email, $password, $cpf, $token, $secure);
$stmt->execute();
